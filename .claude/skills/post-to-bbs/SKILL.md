...
### 4. 處理 BBS 預設分類選單
- **重要：無論發新文或回文，BBS 都會顯示預設分類選單（a)問題 b)情報 c)心得 d)討論 等）**
- **必須直接按 Enter 跳過，不要選擇任何分類（不要輸入 a/b/c/d 等）**
- 原因：
  - 發新文：選擇分類會在標題前加上 `[討論]` 等前綴，與自定義分類 `[AI]` 衝突
  - 回文：選擇分類會破壞 `Re:` 標題格式
- 使用 `bbs_send_key` 發送 `enter` 跳過分類選單

### 4.5. 輸入標題
- 然後輸入標題：
  - 如果有自定義分類參數，組合成：`[分類] 標題`
  - 如果無分類（分類為 `-`），只用標題
  - 如果是回文，標題通常是：`Re: 原標題`
...
### 7. 移動游標到引用原文之後（如果有引用）
- **重要**：如果步驟 5 選擇了引用原文，進入編輯器時引用內容已在檔案中，但游標在檔案開頭
- **必須先移動游標到檔案最後**，才能在引用原文之後輸入回文
- 移動方法：
  - 使用 `bbs_send_ctrl` 發送 `T`（Ctrl+T，跳到檔案結尾）
  - 或者使用 `bbs_send_key` 發送 `end` 鍵（跳到當前行末）然後多次發送 `down` 鍵
- 正確順序：
  ```
  ※ 引述《作者》之銘言：
  > 原文內容
  ...
  本文由 Claude Code 發送
  ```
- **如果沒有引用原文（步驟 5 選 N），則跳過此步驟**，游標已在正確位置
...
### 9. 儲存並發送
- 使用 `bbs_send_ctrl` 發送 `W` (Ctrl+W) 來儲存檔案
- 系統會顯示存檔選單：`[S]存檔 (L)站內 (A)放棄 (T)改標題 (E)繼續 (R/W/D)讀寫刪暫存檔？`
- 發送 `S` 選擇存檔
- 按 Enter 確認
...
2. **禁止使用 Emoji**：BBS 使用 Big5 編碼，不支援 emoji 和特殊 Unicode 符號（如 ✓ ✅ 🎉 等），只能使用基本 ASCII 字符、繁體中文和基本符號（- * + = 等），違反會導致亂碼
3. **錯誤處理**：如果發文過程中遇到錯誤（例如看板不存在、無發文權限），要清楚告知用戶
4. **畫面確認**：每個步驟後都要用 `bbs_get_screen` 確認當前狀態
5. **Big5 編碼**：所有文字都會自動用 Big5 編碼，不需要特別處理
6. **多行內容**：使用 `\n` 來分隔多行內容
