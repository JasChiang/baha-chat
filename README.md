...
從目前文章或看板列表游標出發，收集 seed 文章與後續相關主題文章，回傳適合做討論串摘要的結構化資料。

目前版本：

- 可選 `keyword_search`，會先用 `~` 進入串接模式，再走完整的關鍵字 / 作者 / `m` 標記 / GY 篩選流程
- 可選 `author_prefix`，只用 ID 或作者字首縮小結果；若第一個字是 `*`，可做模糊搜尋
- 可選 `marked_mode`，控制是否只找有 `m` 標記的文章
- 可選 `gy_min`，只找特定 GY 下限以上的文章
- 會先嘗試用 `=` 回到該串 seed 文章
- 再透過文章畫面的 `相關主題` 導航往後抓文
- 盡量把引言、正文、簽名拆開
- 會標記重複觀點，並在重複接話過多時提早停止
- 適合先做 thread collection，再交給上層 agent 產生摘要

### `bbs_summarize_thread`

從目前文章或看板列表游標出發，直接產生討論串摘要草稿，不會自動發文。
...
也支援可選 `keyword_search`，適合只知道關鍵字、不知道篇號時直接先找串。
同樣支援 `author_prefix`、`marked_mode`、`gy_min`，可處理只搜 ID、只搜 GY 或混合條件的情況。
